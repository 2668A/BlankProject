...
void right_auton(){
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{12, 32}, fwd, 40}); 
  chassis.pid_wait();
  ...
  intake_bottom.move(0); 
  intake_middle.move(0); 
  chassis.pid_odom_set({{30.75,18}, rev, 80});
  chassis.pid_wait();
  intake_bottom.move(127); 
  intake_middle.move(127); 
  intake_top.move(127); 
...
void skills_auton(){
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-33,-2},fwd, 90});
  chassis.pid_wait();
  chassis.pid_turn_set(-180,80);
  chassis.pid_wait();
  little_will.set_value(1);
  pros::delay(250);
  chassis.pid_odom_set({{-33,-15},fwd, 70});
  chassis.pid_wait();
  ...
  chassis.pid_wait();
  pros::delay(750);
  chassis.pid_odom_set({{-33,15},rev, 90});
  chassis.pid_wait();
  chassis.odom_xy_set(-33,15);
  intake_bottom.move(-127);
  intake_middle.move(-127);
  pros::delay(250);
  intake_bottom.move(127);
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-20,75},fwd,95});
  chassis.pid_wait();
  chassis.pid_turn_set(-45,100);
  chassis.pid_wait();
  chassis.pid_odom_set({{-33.5,85},fwd,95});
  chassis.pid_wait();
